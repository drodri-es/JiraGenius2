...
function NoClustersFound() {
    return (
        <Card className="flex flex-col items-center justify-center p-8 text-center border-dashed">
            ...
            <h3 className="text-lg font-semibold">No Clear Clusters Found</h3>
            <p className="text-muted-foreground text-sm">
                The AI couldn't identify any distinct groups of similar issues in this project.
            </p>
        </Card>
    )
}
...
export function ClusteringTool() {
  ...
  const handleAnalyze = async () => {
    if (issues.length === 0) {
        toast({ title: 'No issues to analyze', description: 'The selected project has no issues to cluster.' });
        return;
    }
    ...
    try {
        ...
        console.error(e);
        toast({ variant: 'destructive', title: 'Clustering Failed', description: 'An error occurred while analyzing the issues.' });
    } finally {
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Choose the Jira project you want to analyze.</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      {selectedProject && (
        <Card>
          <CardHeader>
            <CardTitle>2. Analyze Issues</CardTitle>
            <CardDescription>
                                ...
                                <AccordionTrigger>
                                    <div className="flex items-center gap-3 text-left">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-muted-foreground">{cluster.issueKeys.length} similar issues</p>
                                        </div>
                                    </div>
                                </AccordionTrigger>
                                <AccordionContent>
